...
int main(int argc, char** argv){
    ...
    MPI_Bcast(&op,len,mpi_op_type,0,MPI_COMM_WORLD);
    ...
    printf("Process %d has chunk_size:%d and displ:%d\n", rank, current_recv_size, displ[rank]);

    
    #pragma omp parallel for default(private) shared(low_vol, epsilon, op) schedule(guided)
    for(j=displ[rank]; j<displ[rank]+current_recv_size; j++){
        for(i=0; i<14; i++)
            op[j].implied_vol[i] =
            newton_raphson(op[j].V_market[i][0], low_vol, epsilon, op[j].S, op[j].K[i], op[j].r, op[j].T, op[j].V_market[i][1]);
    }
    
    MPI_Send(&op[displ[rank]], current_recv_size, mpi_op_type, 0, 0, MPI_COMM_WORLD);
    
    if(rank == 0){
        for (i=0; i<size; i++) {
            current_recv_size = elements_per_proc + (i == size - 1 ? difference : 0);
            MPI_Recv(&toReturn, current_recv_size , mpi_op_type, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for (k=0; k<current_recv_size; k++) {
                for(j=0; j<14; j++) printf("Implied vol. for time %.2f is %.2f%% \n",  (toReturn[k].T), toReturn[k].implied_vol[j]);
            }
        }
        fflush(stdout);
    }
